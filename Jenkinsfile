        ...
        stage('Deploy in UVT Kubernetes') {
            ...
            steps {
                container('helm') {
                    ...
                    sh "helm upgrade --install --force --wait --timeout 600s --kube-context=kubernetes-uvt --namespace integration --set name=${CHART_NAME} --set image.tag=${VERSION} --set domain=${DOMAIN} ${CHART_NAME} -f values-uvt-serrano.yaml ./helm"
                }
            }
        }
    }
